...
const PlayVideo = () => {
  ...
  const fetchOtherData = async () => {
    if (apiData) {
      const channelData_url = `${BASE_URL}/channels?part=snippet%2CcontentDetails%2Cstatistics&id=${apiData?.snippet.channelId}&key=${API_KEY}`;
      await fetch(channelData_url).then(res => res.json()).then(data => setChannelData(data.items[0]));

      const comment_url = `${BASE_URL}/commentThreads?part=snippet%2C%20replies&maxResults=50&videoId=${videoId}&key=${API_KEY}`;
